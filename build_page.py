...
#jeju-on .jo-searchrow{margin:12px 0 0}
#jeju-on .jo-search{width:100%;border:1px solid var(--line);background:#000;color:var(--ink);
  font:inherit;font-size:14px;padding:10px 16px;border-radius:99px}
...
@media (max-width:600px){
  ...
  /* 가로 스크롤로 두면 월 필터·검색창이 화면 밖으로 밀려 안 보인다. 줄바꿈으로 전부 노출. */
  #jeju-on .jo-bar{flex-wrap:wrap;gap:7px;margin:16px 0 0}
  #jeju-on .jo-chip{font-size:12.5px;padding:6px 12px}
  #jeju-on .jo-sep{display:none}
  #jeju-on .jo-searchrow{margin:12px 0 0}
  #jeju-on .jo-search{width:100%;font-size:15px;padding:11px 16px}
...
<div class="jo-searchrow" id="jo-f3"></div>
...
(function () {
  ...
  function drawFilters() {
    ...
    document.getElementById("jo-f3").innerHTML =
      '<input class="jo-search" id="jo-q" type="search" placeholder="행사명 · 장소로 검색" value="' + esc(f.q) + '">';
